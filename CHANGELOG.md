...
- Added game profile catalog validation helpers to verify descriptor id uniqueness, enum uniqueness, lookup consistency, summary count consistency, required/system-list consistency, required/optional system overlap, scenario references, preset validity and checklist generation.
...
- Added `examples/game_profiles.cpp` to show profile discovery, backend-service recommendations, catalog summary, catalog validation, scenario presets, adoption reports and checklist output.
- Added game profile catalog tests for profile lookup, support status, C ABI needs, Action Bridge usage, server-authoritative boundaries, catalog summary, catalog validation, scenario recommendations, adoption reports and checklists.
...
- Exposed the game integration profile catalog, catalog summary, catalog validation, profile scenario recommendations, adoption reports and checklists through the recommended SDK umbrella header.
