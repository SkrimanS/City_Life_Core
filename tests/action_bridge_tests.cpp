...
int main() {
    ...
    const auto valid = clc::sim::dispatch_runtime_action_json(
        engine,
        R"({"action_id":"a1","type":"add_resource","actor_id":"tester","target_id":"riverwatch","resource_id":"grain","amount":5})"
    );
    ...
    const auto invalid_type = clc::sim::dispatch_runtime_action_json(
        engine,
        R"({"action_id":"a2","type":"unknown_action","target_id":"riverwatch","resource_id":"grain","amount":5})"
    );
    ...
    const auto missing_fields = clc::sim::dispatch_runtime_action_json(
        engine,
        R"({"action_id":"a3","type":"add_resource","target_id":"riverwatch"})"
    );
    ...
    const auto remove = clc::sim::dispatch_runtime_action_json(
        engine,
        R"({"action_id":"a4","type":"remove_resource","target_id":"riverwatch","resource_id":"grain","amount":1})"
    );
    ...
    const auto transfer = clc::sim::dispatch_runtime_action_json(
        transfer_engine,
        R"({"action_id":"a5","type":"transfer_resource","target_id":"riverwatch","secondary_target_id":"hillford","resource_id":"grain","amount":2})"
    );
    ...
    const auto advance = clc::sim::dispatch_runtime_action_json(
        transfer_engine,
        R"({"action_id":"a6","type":"advance_days","days":2})"
    );
    ...
    require(json.find("\"action_id\":\"a6\"") != std::string::npos, "result JSON missing action id");
